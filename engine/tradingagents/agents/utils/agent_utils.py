...
def get_china_market_instruction(ticker: str) -> str:
    """返回 A股市场特色规则知识，仅当标的是 A股（.SH/.SZ/.SS 或 6 位数字）。

    注入到市场/基本面分析师的 prompt，使其纳入中国股市独有制度约束，
    避免套用美股逻辑（无涨跌停、T+0、美元计价）给出错误建议。
    """
    t = (ticker or "").strip().upper()
    is_china = (
        t.endswith(".SH") or t.endswith(".SZ") or t.endswith(".SS")
        or (t.isdigit() and len(t) == 6)
    )
    if not is_china:
        return ""
    return """
A股市场特色规则（分析时必须纳入考量，勿套用美股逻辑）：
- 涨跌停板：主板 ±10%、创业板(300/301)与科创板(688) ±20%、ST/*ST ±5%；接近涨跌停时流动性骤降，需评估封板/炸板与次日溢价风险。
- T+1 交易制度：当日买入次日才能卖出，无日内回转，需考虑隔夜风险与仓位约束。
- ST/*ST 风险警示：财务异常或退市风险股，交易受限、退市风险高，必须单独提示。
- 北向资金：沪深股通外资净流入/流出是重要增量资金风向标，结合当日额度使用情况判断。
- 融资融券：两融余额与净买入变化反映杠杆资金情绪。
- 人民币计价（CNY/元）：价格量级与美股（美元）不同，目标价、止损价用元表述。
- 政策面：证监会监管、货币政策、财政政策、产业政策对板块与个股影响显著；国企改革/混改、专精特新是重要主题。
- 交易时段：北京时间 9:30-11:30、13:00-15:00，无盘前盘后（科创板另有盘后固定价交易）。
"""
