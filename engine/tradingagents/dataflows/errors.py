...
class VendorRejectedError(VendorError, ValueError):
    """A vendor explicitly rejected the request with a service-level message
    (余额不足/鉴权失败等), not a symbol problem. Carries the vendor's own text so
    the caller can surface it instead of guessing. Also a ``ValueError`` so the
    HTTP layer's existing ``ValueError`` branch renders it as a readable 400
    (without the "check the symbol" guidance meant for ``NoMarketDataError``).
    """
